...
void bmp_mosaic(unsigned char *imgptr, long long width, long long height, long long size){
    ...
    //computing the real width in bytes polong long of view
    ...
    for (long long i  = 0; i < numbers_width; i++){
        pointer1 = imgptr;
        pointer2= imgptr;
        for (long long k = 0; k < remaining_height; k++){
    ...
    pointer2= imgptr;
    for (long long k = 0; k < remaining_height; k++){
        for (long long j = 0; j < remaining_width; j++){
            sum1 += *pointer1;
            sum2 += *(pointer1+1);
            sum3 += *(pointer1+2);
            pointer1+=3;
    ...
    for (long long i = 0; i < numbers_height; i++){
        for (long long j = 0; j < numbers_width; j++){
            pointer1 = imgptr;
